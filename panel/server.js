...
function generateQrcode() {
      ...
      $.post(taskPostUrl(), (err, resp, data) => {
        try {
          if (err) {
            ...
            $.log(JSON.stringify($.stepsHeaders));
            let setCookie = resp.headers['set-cookie'][0];
            $.log('token', token)
            okl_token = setCookie.substring(setCookie.indexOf("=") + 1, setCookie.indexOf(";"))
            $.log('okl_token', okl_token)          
            $.log("Cookie",cookies)
  ...
  function checkLogin() {
    return new Promise((resolve) => {
      ...
      let loginurl=`https://plogin.m.jd.com/cgi-bin/mm/new_login_entrance?lang=chs&appid=300&returnurl=https://wq.jd.com/passport/LoginRedirect?state=${Ntime}&returnurl=https://home.m.jd.com/myJd/newhome.action?sceneval=2&ufc=&/myJd/home.action&source=wq_passport`
      const options = {
        url: `https://plogin.m.jd.com/cgi-bin/m/tmauthchecktoken?&token=${token}&ou_state=0&okl_token=${okl_token}`,
        body: `lang=chs&appid=300&returnurl=https://wqlogin2.jd.com/passport/LoginRedirect?state=${Ntime}&returnurl=//home.m.jd.com/myJd/newhome.action?sceneval=2&ufc=&/myJd/home.action&source=wq_passport`,
        headers: {
  ...
  function taskPostUrl() {
    let Ntime=(new Date()).getTime();
    const loginUrl=`https://plogin.m.jd.com/cgi-bin/m/tmauthreflogurl?s_token=${s_token}&v=${Ntime}&remember=true`
    return {
